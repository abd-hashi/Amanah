...
public class AdapterOrderItems extends RecyclerView.Adapter<AdapterOrderItems.StoreHolder> {
    ...
    public void onBindViewHolder(@NonNull AdapterOrderItems.StoreHolder holder, int position) {
        ...
        if(data.getType().equals("Custom")){
            ...
            holder.binding.tvPrice.setText( AppConstant.CURRENCY + " " + data.getItem_amount() + " x " + data.getQuantity());

            Glide.with(mContext).load(R.drawable.ic_delivery).into(holder.binding.ivImage);
        }
        else {
        ...
        holder.binding.ivImage.setOnClickListener(v -> {
            if(!data.getType().equals("Custom")) ProjectUtil.imageShowFullscreenDialog(mContext,data.getItem_image());
        });
