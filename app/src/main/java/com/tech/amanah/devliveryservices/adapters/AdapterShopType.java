...
public class AdapterShopType extends RecyclerView.Adapter<AdapterShopType.ViewHolderShopType> {
    ...
    public void onBindViewHolder(@NonNull @NotNull ViewHolderShopType holder, int position) {
        ...
        holder.binding.itemView.setOnClickListener(v -> {
         ...
         if(data.getCategory_name().equalsIgnoreCase("Custom Order") || data.getCategory_name().equalsIgnoreCase("Alabta Kaleh") ){
             mContext.startActivity(new Intent(mContext, CustomOrderAct.class)
                     .putExtra("custom_id", data.getId())
             );
         }
         else {
        ...
        Glide.with(mContext).load(data.getImage()).error(R.drawable.light_gray_circle)
                .placeholder(R.drawable.light_gray_circle).into(holder.binding.ivImg);
