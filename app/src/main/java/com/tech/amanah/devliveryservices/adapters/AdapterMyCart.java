...
public class AdapterMyCart extends RecyclerView.Adapter<AdapterMyCart.StoreCatHolder> {
    ...
    public void onBindViewHolder(@NonNull AdapterMyCart.StoreCatHolder holder, int position) {
        ...
        if(data.getType().equalsIgnoreCase("Custom")) holder.binding.tvName.setText(mContext.getString(R.string.custom_order));
        else  holder.binding.tvName.setText(data.getItem_name());
       if(data.getPriceWithDiscount().equals("")) holder.binding.tvPrice.setText(AppConstant.CURRENCY + " " + String.format("%.2f",Double.parseDouble(data.getItem_price())) + " x " + data.getQuantity());
       else holder.binding.tvPrice.setText(AppConstant.CURRENCY + " " + String.format("%.2f",Double.parseDouble(data.getPriceWithDiscount())) + " x " + data.getQuantity());

        Glide.with(mContext).load(data.getItem_image()).error(R.drawable.ic_delivery).into(holder.binding.ivImage);
